...
class SearchMeta(object):
    def __init__(self, search_api_url):
        # Get core data
        framework_slug = search_api_client.get_index_from_search_api_url(search_api_url)
        self.framework = data_api_client.get_framework(framework_slug)['frameworks']
        ...
        filters = filters_for_lot(current_lot_slug, content_manifest, all_lots=self.framework['lots'])
